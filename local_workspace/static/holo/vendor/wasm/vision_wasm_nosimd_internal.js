var dbg = dbg || function(){ try { console.warn.apply(console, ["[mediapipe]"].concat([].slice.call(arguments))); } catch(e){} }; /* HOLO SHIM: the release glue calls dbg() on its warning path but never defines it — a packaging bug that crashes init instead of logging */
