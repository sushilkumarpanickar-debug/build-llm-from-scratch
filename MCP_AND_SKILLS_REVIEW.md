...
| Telegram, Gmail, Google Calendar | Build connector bridge | Instructions, inbox review, approvals, and agenda | Sender allowlist; Google OAuth read-only; credentials kept locally |
...
`local_workspace/communications.py` supplies the communication bridge. Telegram uses bot polling with an allowed-chat list; Gmail and Google Calendar use read-only OAuth; WhatsApp accepts only signed Cloud API webhook payloads from allowed numbers. External instructions create approval records, and approval stages a local mission rather than carrying out an external action.
